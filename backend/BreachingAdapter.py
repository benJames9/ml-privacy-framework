...
class BreachingAdapter:
    ...
    def setup_image_attack(self, attack_params: AttackParameters = None, cfg=None, torch_model=None):
        print(f"~~~[Attack Params]~~~ {attack_params}")
        ...
        if torch_model is None:
            ...
            #     else visionModels
            # )
            torch_model = self._getTorchModelFromSet(attack_params.model, attack_params.breaching_params.modality)
    ...
    def perform_attack(
        ...
        if cfg.case.data.modality == "vision":
            user.plot(reconstructed_user_data, saveFile=f"reconstructed_data_{user.user_idx}")
        else:
    ...
    def get_metrics(self, num_batches, metrics_arr, cfg, response):
        ...
        for i in range(num_batches):
            with open(f"./attack_images/reconstructed_data_{i}.png", "rb") as image_file:
